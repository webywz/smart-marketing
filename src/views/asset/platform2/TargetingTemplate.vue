<template>
  <div class="targeting-template-container">
    <div class="toolbar">
      <div class="filters">
        <el-input
      ...
      <div class="actions">
        <el-button type="primary" plain @click="syncData">同步定向模板</el-button>
    ...
    >
      ...
      <el-table-column label="定向条件" min-width="250">
...
import type { TargetingTemplate } from '@/mock/platform2Data'

// State
const allData = ref<TargetingTemplate[]>([])
const filteredData = ref<TargetingTemplate[]>([])
const loading = ref(false)
const selectedRows = ref<TargetingTemplate[]>([])
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await request.get('/targetingTemplates')
    allData.value = (res as unknown as TargetingTemplate[]) || []
    filterData()
  } catch (e) {
...
const handleSelectionChange = (val: TargetingTemplate[]) => {
...
const syncData = async () => {
  ...
  setTimeout(async () => {
    try {
      const newItem: TargetingTemplate = {
        id: String(Date.now()),
        name: `同步模板_${new Date().getMinutes()}`,
        product: '同步产品',
        advertiserId: 'ADV_SYNC',
        conditions: ['年龄: 20-30', '地域: 一线城市'],
        createTime: new Date().toISOString().slice(0, 19).replace('T', ' '),
      }
      await request.post('/targetingTemplates', newItem)
...
const handleDelete = (row: TargetingTemplate) => {
  ElMessageBox.confirm('确定删除该模板吗？', '警告', { type: 'warning' })
    .then(async () => {
      try {
        await request.delete(`/targetingTemplates/${row.id}`)
  ...
  ElMessageBox.confirm(`确定删除选中的 ${selectedRows.value.length} 项吗？`, '警告', {
    ...
    .then(async () => {
      try {
        ...
        await Promise.all(ids.map((id) => request.delete(`/targetingTemplates/${id}`)))
